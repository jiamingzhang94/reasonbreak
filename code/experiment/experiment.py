...
# 新加坡专属修复：绕过有缺陷的上传证书流程
os.environ["DASHSCOPE_DISABLE_UPLOAD_CERT"] = "true"  # 关键开关！
dashscope.base_http_api_url = "https://dashscope-intl.aliyuncs.com/api/v1"  # 强制正确端点

# 验证设置
print("DASHSCOPE_DISABLE_UPLOAD_CERT:", os.getenv("DASHSCOPE_DISABLE_UPLOAD_CERT"))
print("base_http_api_url:", dashscope.base_http_api_url)
...
# Add Google Generative AI SDK support
try:
    ...
    print("Warning: google-generativeai package not available. Google Gemini models will not work.")
...
class GeoLocationTester:
    ...
    MODEL_CONFIGS = {
        ...
        'gemini': {
            ...
            'input_token_limit': 2000000,  # Gemini 2.0 has 2M token context
            'supports_reasoning': True
        },
        'sonnet4': {
            'sdk': 'anthropic',
        ...
        'qvq-max': {
            ...
            'api_params': {
                ...
                # 'max_tokens': 8192,
                'sensitive_info_filter': False  # ✅ 关键：禁用敏感信息过滤
            },
...
"""
    ...
    def __init__(self, model='gpt4o', top_n_addresses=1, cot_enabled=True, cot_mode=None, geominer_detector_model='gpt4.1-mini', reasoning_summary='off', prompt_base_defense='off', noise_std=None, root_path='/scratch/zhangjiaming/codes/DoxBench/'):
        # 新加坡专属配置 - 必须放在最开始
        os.environ["DASHSCOPE_INTL"] = "true"
        os.environ["DASHSCOPE_DISABLE_UPLOAD_CERT"] = "true"
        os.environ["NO_PROXY"] = "dashscope-intl.aliyuncs.com"

        # 验证配置
        print(f"【新加坡配置】DASHSCOPE_INTL: {os.getenv('DASHSCOPE_INTL')}")
        print(f"【新加坡配置】DASHSCOPE_DISABLE_UPLOAD_CERT: {os.getenv('DASHSCOPE_DISABLE_UPLOAD_CERT')}")
        ...
        # Ensure root_path ends with a slash for consistent path joining
    ...
    def setup_ai_client(self):
        ...
        elif self.model_config['sdk'] == 'google':
            # Setup Google Generative AI client
            if not GOOGLE_GENAI_AVAILABLE:
                raise ValueError("Google Generative AI SDK not available. Please install: pip install google-generativeai")
...
"""
    ...
    def call_ai_with_custom_prompt(self, image_path, custom_prompt):
        ...
        try:
            ...
            if self.model_config['sdk'] == 'openai':
                ...
                # Note: thinking_config is not supported by all models through OpenAI API
                # Skip adding thinking_config to avoid API errors
                ...
                # Build defense prompt if enabled
                defense_prompt = self.get_defense_system_prompt()
                
                # Combine defense prompt with custom prompt if defense is enabled
                final_prompt = custom_prompt
                if defense_prompt:
                    final_prompt = defense_prompt + "\n\n" + custom_prompt
                ...
                # Build generation config using model-specific parameters
                generation_config = genai.types.GenerationConfig(
                    **self.model_config['api_params']
                )
    ...
    def call_ai_standard(self, image_path):
        ...
        if self.model_config['sdk'] == 'openai':
            ...
            return self.call_google(image_path)
        else:
            raise ValueError(f"Unsupported SDK: {self.model_config['sdk']}")
    ...
    def call_openai_fallback(self, image_path):
        ...
        try:
            ...
            # Extract reasoning content (not needed for models called via OpenAI SDK currently)
            reasoning_content = ""
    ...
    def call_dashscope(self, image_path):
        ...
        try:
            ...
            # 构建请求体 - 使用正确的模型名
            payload = {
                "model": self.model_name,  # 修复：使用配置的模型名而不是硬编码
                "input": {
                    "messages": [
                        {
                            "role": "user",
                ...
                "parameters": self.model_config['api_params']  # 直接使用配置的API参数
            }
            ...
            if response.status_code != 200:
                print(f"Dashscope API call failed with HTTP {response.status_code}: {response.text[:200]}")
                return None, None, api_call_time, None
            ...
            if "output" not in response_data or "choices" not in response_data["output"]:
                print(f"Invalid response structure from Dashscope API")
            ...
            if not choices:
                print("No choices in Dashscope response")
                return None, None, api_call_time, None
            ...
            # 检查答案质量
            if not answer.strip():
                print("Warning: Empty response from Dashscope API")
                # 检查完成原因
                if "finish_reason" in response_data["output"]:
                    finish_reason = response_data["output"]["finish_reason"]
            ...
            return prompt, answer, api_call_time, ""

        except Exception as e:
            print(f"Dashscope API call failed {image_path}: {e}")
            return None, None, None, None
    ...
    def _run_network_diagnostics(self):
        ...
        try:
            ...
            resolver.timeout = 5
            resolver.lifetime = 5
            answers = resolver.resolve('dashscope-intl.aliyuncs.com', 'A')
            ips = [str(rdata) for rdata in answers]
            print(f"DNS 解析成功: {', '.join(ips)}")

            # 测试连接
            import requests
            start = time.time()
            response = requests.get("https://dashscope-intl.aliyuncs.com", timeout=10)
            latency = time.time() - start
            print(f"连接测试: HTTP {response.status_code} (延迟: {latency:.2f}秒)")

            # 测试 TLS 握手
            start = time.time()
            response = requests.get("https://dashscope-intl.aliyuncs.com/api/v1/models", timeout=10)
            tls_time = time.time() - start
            print(f"TLS 握手: {tls_time:.2f}秒 (状态码: {response.status_code})")

        except Exception as e:
            print(f"网络诊断失败: {e}")

    def call_google(self, image_path):
        """Call Google Generative AI API for image analysis"""
        # Unified prompt format (consistent with other models)
        prompt = self.get_standard_prompt()
        ...
        try:
            ...
            # Combine defense prompt with main prompt if defense is enabled
            final_prompt = prompt
            if defense_prompt:
                final_prompt = defense_prompt + "\n\n" + prompt
            ...
            # Extract answer content with safety check
            try:
                answer = response.text if response.text else ""
            except Exception as e:
                print(f"Warning: Failed to extract text from Gemini response for {os.path.basename(image_path)}: {e}")
                answer = ""
            
            # Extract reasoning content for supported models
            reasoning_content = ""
            if self.should_capture_reasoning() and self.model_config.get('supports_reasoning', False):
                # For Google Gemini, we consider the full response as reasoning content
                # since Gemini doesn't provide separate reasoning output like o3/o4mini
                reasoning_content = answer
            
            return prompt, answer, api_call_time, reasoning_content
            
        except Exception as e:
            print(f"Google Generative AI API call failed {image_path}: {e}")
            return None, None, None, None
    ...
    def process_single_image(self, idx, row, total_count):
        ...
        # Build image path
        # image_path = row['filename'].replace('.', 'dataset', 1)  # 旧的硬编码逻辑
        image_path = row['filename']
        
        # Clean up relative path prefixes (remove ./ or ../ at the beginning)
        ...
        if not answer.strip():
            print(f"Warning: Empty response from API for image {os.path.basename(row['filename'])}")
        elif not address_list.strip():
            print(f"Warning: No address list found in response for image {os.path.basename(row['filename'])}")
    ...
    def process_csv_parallel(self, input_csv_path, output_csv_path=None, max_workers=1, batch_size=10, max_tasks=None, random_sample=None, random_seed=None, breakpoint=None):
        ...
        try:
            ...
            # df['filename'] = df['filename'].str.replace(r'^\./', '../../dataset/', regex=True)
    ...
    if uploaded_file is not None:
        ...
        if st.button("Start Test"):
            ...
            try:
                ...
                # For Streamlit mode, use current working directory as default root path
                tester = GeoLocationTester(model=model_choice, top_n_addresses=top_n_choice, cot_mode=cot_mode, geominer_detector_model=geominer_detector_model_choice, prompt_base_defense=prompt_base_defense, root_path=os.getcwd())
...
def main():
    ...
    if len(os.sys.argv) > 1 and os.sys.argv[1] == "streamlit":
        ...
        parser.add_argument("--input_csv", default="../dataset/ori.csv", help="Input CSV file path")
        ...
        parser.add_argument("--model",
                          ...
                          metavar="MODEL")        # Add model selection parameter

        parser.add_argument("--prompt_base_defense",
                          type=str,
                          default='off')
        ...
        # Add root path parameter
        parser.add_argument("--root_path",
                          type=str,
                          default="/scratch/zhangjiaming/codes/DoxBench/",
                          help="Root directory path for resolving relative image paths in CSV (default: /scratch/zhangjiaming/codes/DoxBench/)",
                          metavar="PATH")
        ...
        try:
            # Initialize tester, passing selected model, top_n_addresses, cot_mode, geominer_detector_model, reasoning_summary, prompt_base_defense, noise_std and root_path
            tester = GeoLocationTester(model=args.model, top_n_addresses=top_n_addresses, cot_mode=cot_mode, geominer_detector_model=args.geominer_detector_model, reasoning_summary=args.reasoning_summary, prompt_base_defense=args.prompt_base_defense, noise_std=args.noise, root_path=args.root_path)
